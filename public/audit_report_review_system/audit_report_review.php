<?php
    if($_SERVER["REQUEST_METHOD"] === "POST") {
		...
		elseif($_POST["action"] == "review_audit_report"):
			// dump($_POST);
			...
			if(isset($_REQUEST["status"])):
				if($_REQUEST["status"] != ""):
					if($_REQUEST["status"] == "CREATE"):
						$where .= " AND ar.audit_report_status IS NULL"; // Handle NULLs
					else:
						$where .= " AND ar.audit_report_status = '".$_REQUEST["status"]."'";
					endif;
				endif;
			endif;

			$grouper_string = " GROUP BY aps.aps_id, aa.area_id order by audit_report_status asc";
...
<br>
		...
		if(!isset($_GET["action"])):
			render("public/audit_report_review_system/ar_review_list.php",[
			]);
		else:
