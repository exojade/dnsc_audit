...
class GoogleClient
{
    ...
    public function __construct()
    {
        ...
        if (isset($_SESSION["dnsc_audit"]['GOOGLE_ACCESS_TOKEN'])) {
            $this->client->setAccessToken($_SESSION["dnsc_audit"]['GOOGLE_ACCESS_TOKEN']);
            //  Checking current access token is expired
            if($this->client->isAccessTokenExpired()){
                // Refresh access token and add it into session
                $client->refreshToken($_SESSION["dnsc_audit"]['GOOGLE_REFRESH_TOKEN']);
                $access_token = $this->client->getAccessToken();
                $_SESSION["dnsc_audit"]['GOOGLE_ACCESS_TOKEN'] = $access_token;
            }
        }
    }
    ...
    public function authenticate($code)
    {
        ...
        $_SESSION["dnsc_audit"]['GOOGLE_ACCESS_TOKEN'] = $this->client->getAccessToken();
        $_SESSION["dnsc_audit"]['GOOGLE_REFRESH_TOKEN'] =  $this->client->getRefreshToken();
    }
